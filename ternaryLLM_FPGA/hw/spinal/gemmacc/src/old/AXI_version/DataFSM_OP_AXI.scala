...
class DataFSM_OP_AXI(conf: ConfigSys) extends Component {
...
// TODO: Asynch read triggered bin for initialize Buffer_X
  ...
  val cnt_l = Counter(conf.ENTRIES_PER_BEAT_X_Y)
  ...
  // Assign values to kernel stream
    for (i <- 0 until conf.ENTRIES_PER_BEAT_W) {
      for(j <- 0 until conf.UNROLL_M){
        io.kernel(j)((cnt_w.value * conf.ENTRIES_PER_BEAT_W + i).resize(conf.BITWIDTH_S)).payload := io.AXI.r.data.subdivideIn(conf.ENTRIES_PER_BEAT_W slices)(i).asUInt
      }
  }
  ...
  val dataFSM = new StateMachine {
    ...
    LOAD_X_Buffer.whenIsActive {
      ...
      io.AXI.ar.addr := (io.base_addr_X + (cnt_unroll_rows.value * conf.UNROLL_M * (io.K << conf.BYTE_X))).resize(conf.axiConfig.addressWidth)  // BURST READ of whole UNROLL_M
      io.AXI.ar.len := ((conf.UNROLL_M * io.K * conf.BIT_WIDTH_X_Y) / conf.axiConfig.dataWidth).resize(conf.axiConfig.lenWidth) - 1 // READ whole UNROLL M rows at once
    ...
    LOAD_X_DATA.whenIsActive {
      ...
      addr_w := (row_c.value * conf.BUFFERSIZE_PER_ROW + k_c.value).resize(conf.ADDR_WIDTH_BUFFER)
    ...
    SET_ADDR.whenIsActive{
      addr_r := (cnt_row.value * conf.BUFFERSIZE_PER_ROW + k_c.value * conf.BUFFER_ENTRIES_PER_KSLICE + cnt_s.value).resize(conf.ADDR_WIDTH_BUFFER)
      goto(LOAD_SLICE)
    ...
    LOAD_SLICE.whenIsActive {
      ...
      for (l <- 0 until conf.ENTRIES_PER_BEAT_X_Y) {
        x_reg(cnt_row.value)((cnt_s.value * conf.ENTRIES_PER_BEAT_X_Y + l).resize(log2Up(conf.K_slice))) := Buffer_Out(l)
      }
    ...
    MEM_ADDR_Y.whenIsActive {
      ...
      io.AXI.aw.addr := (io.base_addr_Y +  ((((cnt_unroll_rows.value * conf.UNROLL_M ) + cnt_unroll.value) * (io.N << conf.BYTE_Y)) + (cnt_N_write << conf.BYTE_Y))).resize(conf.axiConfig.addressWidth)
    ...
    MEM_WRITE_Y.whenIsActive {
      ...
      // AXI-W Configuration
      io.AXI.w.data := (io.result_GEMM((cnt_unroll.value).resize(log2Up(conf.UNROLL_M))).reverse).map(_.asBits).reduce(_ ## _).subdivideIn(conf.axiConfig.dataWidth bits)(cnt_beats_y.resize(log2Up(conf.total_numb_beat_Y)))
